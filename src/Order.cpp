...
void Order::Behavior() {
    ...
    // if there is no chef available, order is waiting
    if (this->data.get_chefs_store()->Used() == this->data.get_chef_num()) {
        int i = 0;
        while (i < intern_time::in_hours(1)) {
            // every second check chef store
    ...
    double cooking_time = Normal(intern_time::in_minutes(this->data.get_chef_center()),
                                 intern_time::in_minutes(this->data.get_chef_sigma()));
...
void Order::delivering_internal() {
    ...
    if (this->data.get_car_type() == DIESEL) {
        consumption =
                (DIESEL_CONSUMPTION * (CITY_SPEED * intern_time::to_hours(delivery_time * 1.7))) / REFERENCE_DISTANCE;
    } else {
        consumption =
